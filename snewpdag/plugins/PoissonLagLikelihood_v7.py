...
PoissonLagLikelihood_v7           : Calculate the Poisson likelihood of a histogram pair corresponding to a certain time lag.
                                    ...
                                    The workflow logic of v7 is the same as v6.
...
class PoissonLagLikelihood_v7(Node):
  ...
  def alert(self, data):
    ...
    if 'hist1' in hist_data and 'hist2' in hist_data:
      ...
      result = {
        ...
        'sum_lnJ_list': np.asarray(lnJ, dtype=np.float64),
        'sum_ln_fac_list': np.asarray(ln_fac, dtype=np.float64)
      }
    ...
    result = {
        ...
        'sum_lnJ': float(sum_lnJ),
        'sum_ln_fac': float(sum_ln_fac)
    }
    store_field(data, self.out_field, result)
    return True
