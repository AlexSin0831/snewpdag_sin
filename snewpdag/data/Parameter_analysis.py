...
Fixing SuperK yield (7800 / 4000), then vary alpha (det2's yield / SuperK's yield) and sigma of the GND
"""
...
MC_OUTPUT_DIRECTORY = PROJECT_ROOT / 'output' / 'analysis_yield'
...
SUPERK_RESOL = 1e-6
ALPHA = 1 # det2's yield / det1's yield 
...
def summary_builder(coarse_data, fine_data):
  ...
  total_ln_fac = np.concatenate((coarse_ln_fac[coarse_outside_roi], fine_ln_fac))
...
def main():
  ...
  data = build_timeseries(args.true_lag, args.superk_yield, args.superk_bg, 
                          args.alpha,
                          model_directory, background_window_1=(-1.0,9.0), background_window_2=(-1.0,9.0))

  # Be careful, that now scan_data is a dictionary, saving scanning data of coarse, fine and total scan.
  scan_data = calculations(data, args.superk_yield, args.superk_bg, 
                           args.alpha,
                          args.coarse_lag_step,
                          args.fine_lag_step,
                          args.hist_bin_width,
