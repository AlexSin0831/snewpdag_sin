...
# Variance / Expected
sigma_fudge = float(np.sqrt(0.0149 / 0.0162))
  ...
  for path in sorted(results_dir.glob('trial_*.csv')):
    with path.open(newline='') as f: # f is the opened file object (just a temporary variable name)
      ...
      for row in reader:
        ...
        row['sigma3'] = float(row['sigma1']) * float(sigma_fudge)
        row['pull3'] = float(row['pull1']) / float(sigma_fudge)
        rows.append(row)
        for fieldname in list(reader.fieldnames or []) + ['source_file'] + ['sigma3']:
...
def plot_pull_distribution(rows, pull_plot, bins):
  method_specs = (
      ('pull3', 'Godambe Information Method', 'tab:orange', 'godambe_info'),
  )
  pulls_by_method = {}
...
def main():
  ...
  if args.combined_csv is None:
    args.combined_csv = args.run_dir / 'summary_godambe.csv'

  if args.pull_plot is None:
    args.pull_plot = args.run_dir / 'pull_distribution_godambe.png'
  ...
  if summary is not None:
    for field, label in (('pull3', 'Godambe Method'),):
      if field in summary:
