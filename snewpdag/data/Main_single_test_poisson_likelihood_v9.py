...
MC_OUTPUT_DIRECTORY = PROJECT_ROOT / 'output' / 'mc_v9_test'
...
def calculations(payload_data,
  ...
  if det1 != 'IceCube':
    ...
    if not np.any(valid_indices_1):
      raise ValueError(
          'No positive-lambda calibration points are available for {}'.format(det1)
      )
    if not np.any(valid_indices_2):
      raise ValueError(
          'No positive-lambda calibration points are available for {}'.format(det2)
      )
  ...
  print('coefficient of 1/lambda of {} = {:.4f}'.format(det1, c1_cal))
  print('coefficient of 1/lambda of {} = {:.4f}'.format(det2, c2_cal))
