...
MC_OUTPUT_DIRECTORY = PROJECT_ROOT / 'output' / 'test_mc_v5'
...
WINDOW_SIZE = 2.0 #s
# we intentionally extend the time series to include more background, to avoid meaningless comparison between 2 detectors. 
# don't be too large, otherwise the computation will be extremely long. 
BACKGROUND_START = -0.5 # s
BACKGROUND_STOP = 2.5 # s
SEED = 12345
...
def build_timeseries(true_lag, detector_pair, model_directory_1, model_directory_2,
                     background_window_1=(BACKGROUND_START, BACKGROUND_STOP),
                     background_window_2=(BACKGROUND_START, BACKGROUND_STOP)):
...
def shift_time_series_to_opposite_lag(ts1, ts2, time_lag):
  """Keep detector order fixed, but move ts2 from +lag to -lag."""
  shifted_ts2 = TimeSeries(ts2.start, ts2.stop)
  shifted_ts2.add(np.asarray(ts2.times, dtype=np.float64) - 2.0 * float(time_lag))
  return {
      'ts1': ts1,
      'ts2': shifted_ts2,
  }
...
def calculations(payload_data,
  ...
  # Background has already been generated in the detector time series.
...
def build_plot_filename(detector_pair, true_lag, coarse_time_lag_step_size,
  ...
  if trial_label is not None:
    label = '_{}'.format(format_text_for_filename(trial_label))
  return (
    'LogL_against_tau'
    '_DETS-{}-{}'
    '_hist-bin-width-{}s'
    ...
    '_seed-{}{}.png'
           ...
           scan_low_text, scan_high_text, coarse_lag_step, fine_lag_step,
           seed, label)
...
def parse_args():
  ...
  parser.add_argument('--background-start', type=float, default=BACKGROUND_START,
                      help='Start time for generated detector background.')
  parser.add_argument('--background-stop', type=float, default=BACKGROUND_STOP,
                      help='Stop time for generated detector background.')
...
def build_result_filename(detector_pair, seed, trial_label=None):
  det1 = format_text_for_filename(detector_pair[0])
  det2 = format_text_for_filename(detector_pair[1])
  label = ''
  if trial_label is not None:
    label = '_{}'.format(format_text_for_filename(trial_label))
  return 'trial_{}_{}_seed-{}{}.csv'.format(det1, det2, seed, label)
...
def save_trial_result(args,
  ...
  result_filename = args.results_dir / build_result_filename(
      detector_pair, args.seed, trial_label)
  if true_lag is None:
    true_lag = args.true_lag

  if standard_deviation > 0.0 and np.isfinite(standard_deviation):
    pull = (best_lag - true_lag) / standard_deviation
  else:
    pull = np.nan
...
def main():
  args = parse_args()
  if args.background_stop <= args.background_start:
    raise ValueError('background stop must be larger than background start')

  detector_pair = (args.det1, args.det2)
  opposite_true_lag = -args.true_lag
  ...
  background_window = (args.background_start, args.background_stop)
  data_1 = build_timeseries(args.true_lag, detector_pair, model1, model2,
                            background_window_1=background_window,
                            background_window_2=background_window)

  ts1_original, ts2_original = data_1['ts1'], data_1['ts2']
  opposite_time_series = shift_time_series_to_opposite_lag(
      ts1_original, ts2_original, args.true_lag)
  data_2 = data_1.copy()
  data_2['ts1'], data_2['ts2'] = opposite_time_series['ts1'], opposite_time_series['ts2']

  # Be careful, that now scan_data is a dictionary, saving scanning data of coarse, fine and total scan.
  scan_data_1 = calculations(
    data_1,
    args.coarse_lag_step,
    args.fine_lag_step,
    args.hist_bin_width,
  ...
  output_filename_1 = args.output_dir / build_plot_filename(
    detector_pair, args.true_lag, args.coarse_lag_step,
    args.fine_lag_step, args.hist_bin_width, args.window_size,
    args.scan_low, args.scan_high, args.seed, trial_label='forward'
  )

  output_filename_2 = args.output_dir / build_plot_filename(
    detector_pair, opposite_true_lag, args.coarse_lag_step,
    args.fine_lag_step, args.hist_bin_width, args.window_size,
    args.scan_low, args.scan_high, args.seed, trial_label='opposite-lag'
  )

  best_lag_1, standard_deviation_1 = plot_scan_and_find_best_lag(scan_data_1, args.true_lag, output_filename_1, detector_pair, args.hist_bin_width, args.window_size)
  best_lag_2, standard_deviation_2 = plot_scan_and_find_best_lag(scan_data_2, opposite_true_lag, output_filename_2, detector_pair, args.hist_bin_width, args.window_size)

  # Generate a csv file to save stuff!
  result_filename_1, pull_1 = save_trial_result(
      args, detector_pair, best_lag_1, standard_deviation_1,
      output_filename_1, true_lag=args.true_lag, trial_label='forward')
  result_filename_2, pull_2 = save_trial_result(
      args, detector_pair, best_lag_2, standard_deviation_2,
      output_filename_2, true_lag=opposite_true_lag, trial_label='opposite-lag')

  print('---------------------------------------')
  print('Summary:')
  print('detectors: {} and {}'.format(detector_pair[0], detector_pair[1]))
  print('histogram bin width: {:.6f} s'.format(args.hist_bin_width))
  print('background window: {:.6f} s to {:.6f} s'.format(
      args.background_start, args.background_stop))
  ...
  print('true lag_1: {:.6f} s'.format(args.true_lag))
  print('true lag_2: {:.6f} s'.format(opposite_true_lag))
  print('best lag_1: {:.6f} s'.format(best_lag_1))
  print('best lag_2: {:.6f} s'.format(best_lag_2))
  print('sigma_1: {:.6f} s'.format(standard_deviation_1))
  print('sigma_2: {:.6f} s'.format(standard_deviation_2))
  print('pull: {:.6f}'.format(pull_1))
  print('pull: {:.6f}'.format(pull_2))
  print('plot: {}'.format(output_filename_1))
  print('plot: {}'.format(output_filename_2))
  print('result csv: {}'.format(result_filename_1))
  print('result csv: {}'.format(result_filename_2))
