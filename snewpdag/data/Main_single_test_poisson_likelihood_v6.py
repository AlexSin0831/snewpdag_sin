...
PoissonLagLikelihood_v6 ==> Applied interpolation to the lnJ table
...
def calculations(payload_data,
  ...
  # Not running yet, we are preparing the tools here first:
  if TOY == True: 
    smoothed_pair = PairTimeSeriesToHist_Smoothing_Toy('ts1',
                                                        'ts2',
                                                        'hist_pair',
    ...
    smoothed_pair = PairTimeSeriesToHist_Smoothing('ts1',
                                                    'ts2',
                                                    'hist_pair',
  ...
  for lag in payload_data['possible_time_lag_list']:
    ...
    smoothed_pair.update(temp_data)
    like_cal.update(smoothed_pair.last_data)
...
# LIKELIHOOD PLOT:
def plot_scan_and_find_best_lag(scan_data, true_lag, filename, detector_pair, histogram_bin_width):
  # Retrieving the data:
...
def main():
  ...
  best_lag, standard_deviation = plot_scan_and_find_best_lag(scan_data, args.true_lag, output_filename, detector_pair, args.hist_bin_width)
