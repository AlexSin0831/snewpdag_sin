...
PairTimeSeriesToHist ==> Build histogram pairs without smoothing
...
PoissonLagLikelihood_v6 ==> New integration by part approach ; Use bilinear interpolation on the lnJ table
...
PoissonLagLikelihood = load_plugin('PoissonLagLikelihood_v6.py').PoissonLagLikelihood_v6
...
MC_OUTPUT_DIRECTORY = PROJECT_ROOT / 'output' / 'mc_v6_test'
...
def build_timeseries(true_lag, detector_pair, model_directory_1, model_directory_2,
                     background_window_1=(-3.0, 3.0),
                     background_window_2=(-3.0, 3.0),
...
def plot_scan_and_find_best_lag(scan_data, true_lag, filename, detector_pair,
                                histogram_bin_width, smoothing, toy):
  # Retrieving the data:
  ...
  print('Plotting Info:')
  print('raw combined-scan best lag: {:.6f} s'.format(best_lag))
  print('standard error from 2nd derivative method: {:.6f} s'.format(
      standard_error_1))
  print('standard error from 0.5 method: {:.6f} s'.format(
      standard_error_2))
  ...
  if not smoothing:
    ax.set_title('{} vs {}, binwidth={}, NO SMOOTHING'.format(
        detector_pair[0], detector_pair[1], histogram_bin_width))
    kernel_legend_title = None
  elif toy:
    ax.set_title('{} vs {}, binwidth={}, kernel-type = Gaussian'.format(detector_pair[0], detector_pair[1], histogram_bin_width))
    ...
    ax.set_title('{} vs {}, binwidth={}, kernel-type = Convolution ({})'.format(detector_pair[0],
                                                                                detector_pair[1],
                                                                                histogram_bin_width,
...
def apply_smoothing_args(args):
  ...
  # after parsing so --hist1-ic cannot enable it for another detector;
  # --no-hist1-ic remains available for IceCube when needed.
...
def save_trial_result(args,
  ...
  row = {
      ...
      'best_lag': best_lag,
      # Keep the original names as aliases for existing v6 consumers.
...
def main():
  ...
  bg_window_1, bg_window_2 = detector_background_windows(
      args.window_start, args.window_size, args.scan_low, args.scan_high)
  ...
  (best_lag, standard_error_1, standard_error_2,
   left_error_bound_2, right_error_bound_2) = plot_scan_and_find_best_lag(
      ...
      args.hist_bin_width, args.smoothing, args.toy)
  ...
  print('window: {:.6f} s to {:.6f} s'.format(
      args.window_start, args.window_start + args.window_size))
  print('detector 1 generated as histogram: {}'.format(args.hist1_ic))
  if not args.smoothing:
    print('smoothing: disabled')
  elif args.toy:
    print('smoothing kernel: Gaussian '
          '(sigma={:.6f} s, impact range={:.6f} s)'.format(
