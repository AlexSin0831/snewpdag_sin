...
PoissonLagLikelihood_v4 ==> New integration by part approach
...
def build_total_likelihood_table(coarse_scan, fine_scan):
  """Combine a full-range coarse scan with its fine-scan ROI.
...
# SETUP THE PAYLOAD:
# BUILD 2 DETECTOR INPUTS, WHICH WILL BE FED INTO OUR TRANSFORMER LATER:
...
def plot_scan_and_find_best_lag(scan_data, true_lag, filename, detector_pair, histogram_bin_width, window_size):
  ...
  ax.set_title('{} vs {}, binwidth={}, NO SMOOTHING'.format(detector_pair[0],
                                                            detector_pair[1],
                                                            histogram_bin_width,
                                                            ))
  ax.legend()
...
def main():
  ...
  bg_window_1, bg_window_2 = detector_background_windows(
      0.0, args.window_size, args.scan_low, args.scan_high)
